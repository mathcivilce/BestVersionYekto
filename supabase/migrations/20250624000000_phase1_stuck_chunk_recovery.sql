...
BEGIN
    ...
    IF v_count > 0 THEN
        ...
        -- Insert recovery event for tracking (if table exists)
        BEGIN
...
-- ============================================================================================================
-- 1.9 IMMEDIATE STUCK CHUNK RECOVERY (TESTING)
-- ============================================================================================================

-- Test the recovery system and fix any stuck chunks immediately
DO $$
DECLARE
    v_test_result RECORD;
BEGIN
    -- Test stuck chunk detection and recovery
    SELECT * INTO v_test_result FROM reset_stuck_chunks(5);
    
    RAISE NOTICE '🚀 PHASE 1 IMPLEMENTATION COMPLETE:';
    RAISE NOTICE '✅ Stuck chunks found and reset: %', v_test_result.reset_count;
    ...
    IF v_test_result.reset_count > 0 THEN
        ...
        RAISE NOTICE '📋 Chunk details: %', v_test_result.recovery_details;
    END IF;
    
    RAISE NOTICE '🔧 Your sync system should now automatically recover from stuck chunks!';
END;
$$; 